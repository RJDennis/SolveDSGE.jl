...
%TCIDATA{LastRevised=Saturday, May 02, 2020 16:11:05}
...
\title{SolveDSGE v0.3.5 --- A User Guide}
\date{April 2020}
\maketitle
...
approximation is based on Fourier series (Kronmal and Tarter, 1968). \ To
approximate the probability density function the function is

\bigskip 

\textit{nodesf, f =
approximate\_denisty(sample,order,lower\_bound,upper\_bound)}

\bigskip 

where \textit{sample} is a vector of data, \textit{order} is the order of
the Fourier series approximation, and \textit{lower\_bound} and \textit{%
upper\_bound} specify the support over which the PDF is constructed. \
Similarly, the cumulative distribution function is approximated using the
function
...
where \textit{sample} is a vector of data, \textit{order} is the order of
the Fourier series approximation, and \textit{lower\_bound} and \textit{%
upper\_bound} specify the support over which the CDF is constructed.
