...
function solve_nonlinear(model::REModel,scheme::ChebyshevSchemeDet)
    ...
    while len > scheme.tol_variables && iters <= scheme.maxiters
        ...
        for i = 1:N
            ...
            ind = Tuple(CartesianIndices(Tuple(length.(grid)))[i])
            for j = 1:nx
            ...
            soln = nlsolve(projection_equations, init, xtol = scheme.tol_fix_point_solver, iterations = scheme.maxiters, inplace = false)
            for j = 1:nv
                new_variables[j][ind...] = soln.zero[j]
            end
        end
...
function solve_nonlinear(model::REModel,soln::R,scheme::ChebyshevSchemeDet) where {R <: PerturbationSolutionDet}
    ...
    for j = 1:N
        ind = Tuple(CartesianIndices(Tuple(length.(grid)))[j])

        for i = 1:nx
            state[i] = grid[i][ind[i]]
        end

        for i = 1:ny
            variables[i][ind...] = gbar[i] + (gx[i:i,:]*(state - hbar))[1]
        end
        for i = 1:nx
            variables[ny+i][ind...] = hbar[i] + (hx[i:i,:]*(state - hbar))[1]
        end

        if typeof(soln) <: SecondOrderSolutionDet
            for i = 1:ny
                variables[i][ind...] += (1/2)*sum(vec(soln.gxx[(i-1)*nx+1:i*nx,:]).*kron((state - hbar),(state - hbar)))
            end
            for i = 1:nx
                variables[ny+i][ind...] += (1/2)*sum(vec(soln.hxx[(i-1)*nx+1:i*nx,:]).*kron((state - hbar),(state - hbar)))
            end
        end

        if typeof(soln) <: ThirdOrderSolutionDet
            for i = 1:ny
                variables[i][ind...] += (1/2)*((soln.gxx[i:i,:])*kron((state - hbar),(state - hbar)))[1] + (1/6)*(soln.gxxx[i:i,:]*kron(kron((state - hbar),(state - hbar)),(state-hbar)))[1]
            end
            for i = 1:nx
                variables[ny+i][ind...] += (1/2)*((soln.hxx[i:i,:])*kron((state - hbar),(state - hbar)))[1] + (1/6)*(soln.hxxx[i:i,:]*kron(kron((state - hbar),(state - hbar)),(state-hbar)))[1]
            end
        end
...
function solve_nonlinear(model::REModel,soln::R,scheme::ChebyshevSchemeStoch) where {R <: PerturbationSolutionStoch}
    ...
    for j = 1:N
        ...
        if typeof(soln) <: SecondOrderSolutionStoch
            for i = 1:ny
                variables[i][ind...] += (1/2)*soln.gss[i] + (1/2)*sum(vec(soln.gxx[(i-1)*nx+1:i*nx,:]).*kron((state - hbar),(state - hbar)))
            end
            for i = 1:nx
                variables[ny+i][ind...] += (1/2)*soln.hss[i] + (1/2)*sum(vec(soln.hxx[(i-1)*nx+1:i*nx,:]).*kron((state - hbar),(state - hbar)))
            end
        end

        if typeof(soln) <: ThirdOrderSolutionStoch
            for i = 1:ny
                variables[i][ind...] += (1/2)*soln.gss[i] + (1/2)*((soln.gxx[i:i,:])*kron((state - hbar),(state - hbar)))[1] + (3/6)*(soln.gssx[i:i,:]*(state - hbar))[1] + (1/6)*(soln.gxxx[i:i,:]*kron(kron((state - hbar),(state - hbar)),(state-hbar)))[1]
            end
            for i = 1:nx
                variables[ny+i][ind...] += (1/2)*soln.hss[i] + (1/2)*((soln.hxx[i:i,:])*kron((state - hbar),(state - hbar)))[1] + (3/6)*(soln.hssx[i:i,:]*(state - hbar))[1] + (1/6)*(soln.hxxx[i:i,:]*kron(kron((state - hbar),(state - hbar)),(state-hbar)))[1]
            end
        end
    ...
    for i = 1:nv
        ...
        if typeof(soln) <: ChebyshevSolutionDet
            ...
            for j = 1:N
                ...
                for l = 1:nx
                    state[l] = grid[l][ind[l]]
                end

                variables[i][ind...] = chebyshev_evaluate(w,state,soln.order,domain)
        ...
        elseif typeof(soln) <: SmolyakSolutionDet
            ...
            for j = 1:N
                ...
                variables[i][ind...] = smolyak_evaluate(w,state,soln.multi_index,domain)
        ...
        elseif typeof(soln) <: PiecewiseLinearSolutionDet
            ...
            for j = 1:N
                ...
                variables[i][ind...] = piecewise_linear_evaluate(soln_variables[i],soln.nodes,state)
...
function solve_nonlinear(model::REModel,scheme::PiecewiseLinearSchemeDet)
    ...
    while len > scheme.tol_variables && iters <= scheme.maxiters
        ...
        for i = 1:N
            ...
            point = Array{T,1}(undef,length(init))
            soln = nlsolve(projection_equations, init, xtol = scheme.tol_fix_point_solver, iterations = scheme.maxiters, inplace = false)
            for j = 1:nv
                new_variables[j][ind...] = soln.zero[j]
            end
