...
function decision_rule(soln::R1) where {R1 <: Union{FirstOrderSolutionDet,FirstOrderSolutionStoch}}
...
function decision_rule(soln::R1) where {R1 <: Union{SecondOrderSolutionDet,SecondOrderSolutionStoch}}
...
function decision_rule(soln::R1) where {R1 <:ThirdOrderSolutionDet}
    ...
    function create_decision_rule(state::Array{T,1}) where {T <: AbstractFloat}
        ...
        for i = 1:ny
          y[i] = soln.gbar[i] + (soln.gx[i:i,:]*(state-soln.hbar))[1] 
               + (1/2)*sum(vec(soln.gxx[(i-1)*nx+1:i*nx,:]).*kron((state - soln.hbar),(state - soln.hbar)))
               + (1/2)*((soln.gxx[i:i,:])*kron((state - soln.hbar),(state - soln.hbar)))[1] + (1/6)*(soln.gxxx[i:i,:]*kron(kron((state - soln.hbar),(state - soln.hbar)),(state - soln.hbar)))[1]
        end
...
function decision_rule(soln::R1) where {R1 <:ThirdOrderSolutionStoch}
    ...
    function create_decision_rule(state::Array{T,1}) where {T <: AbstractFloat}
        ...
        for i = 1:ny
          y[i] = soln.gbar[i] + (soln.gx[i:i,:]*(state-soln.hbar))[1] 
               + (1/2)*(soln.gxx[i:i,:].*kron((state - soln.hbar),(state - soln.hbar)))[1]
               + (1/2)*soln.gss[i] + (1/2)*((soln.gxx[i:i,:])*kron((state - soln.hbar),(state - soln.hbar)))[1] + (3/6)*(soln.gssx[i:i,:]*(state - soln.hbar))[1] + (1/6)*(soln.gxxx[i:i,:]*kron(kron((state - soln.hbar),(state - soln.hbar)),(state - soln.hbar)))[1]
        end
...
function decision_rule(soln::R1) where {R1 <: Union{ChebyshevSolutionDet,ChebyshevSolutionStoch}}
    ...
    function create_decision_rule(state::Array{T,1}) where {T <: AbstractFloat}
        ...
        ny = length(soln.weights)

        y  = zeros(ny)
        
        for i = 1:ny
          y[i] = chebyshev_evaluate(soln.weights[i],state,soln.order,soln.domain)
        end
...
function decision_rule(soln::R1) where {R1 <: Union{SmolyakSolutionDet,SmolyakSolutionStoch}}

    function create_decision_rule(state::Array{T,1}) where {T <: AbstractFloat}

        if length(state) != size(soln.grid,2)
            error("state vector has incorrect size")
        end
        ...
        for i = 1:ny
          y[i] = smolyak_evaluate(soln.weights[i],state,soln.multi_index,soln.domain)
        end
...
function decision_rule(soln::R1) where {R1 <: Union{PiecewiseLinearSolutionDet,PiecewiseLinearSolutionStoch}}
